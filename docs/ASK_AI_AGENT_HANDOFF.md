...
If semantic terminology providers fail and the user explicitly supplied every requested category, the server produces an exact-wording evidence artifact rather than a total failure. It preserves zero-count categories and clearly states that synonyms were not inferred. Requests that require an AI-derived ontology still fail transparently rather than receiving invented or unrelated counts.
