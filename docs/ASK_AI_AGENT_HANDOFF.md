...
Implementation state: the original transaction protocol was introduced in `7c730f6`; the current repository HEAD includes the July 15 Ask AI reliability, fallback-key, whole-history, UI, and liberal-planning hardening plus the July 16 deterministic analytics/action/follow-up fixes described below.
...
The plan binds named analytical scopes and measures rather than merely recording that analytics was requested. A comparison such as “past seven days versus the seven days before” resolves two named periods, loads one 14-day bounded union, and groups the calculation by scope. Supported factual analytics are provider-independent terminal results: the server composes the answer, coverage statement, missing-data policy, evidence ledger, optional visual, and clickable dates without calling the reranker or a language model. Models remain available for genuinely open-ended interpretation, not basic arithmetic.

An analytics plan may contain several measures with independent operations and coverage requests. “Average pain and maximum energy” does not force both fields into one aggregation, while “average pain for recorded days plus the unlogged-day count” returns the calculated average, observed count, missing count, and missing-data policy from one bounded server execution. Rolling wording such as “past month” resolves to a bounded 30-day scope; explicit calendar wording such as “this month,” “last calendar month,” and named months resolves to calendar-month bounds. Explicit comparison periods remain separately named inside their bounded union.

Prompt construction is capability-specific. History reads receive bounded/ranked evidence but not mutation schemas or unrelated app configuration; analytics interpretation receives the verified server result; exercise questions receive relevant exercises; and action planning receives a compact action contract plus relevant entity targets. Follow-up artifacts are carried as structured state, not as raw prior assistant prose, so flags such as `visualization:false` cannot create a new visualization intent. Debug metadata records the selected prompt profile and projected character size. This prevents a small question from becoming a universal prompt merely because the route supports many capabilities.

Deterministic action planning is clause-aware. Independent commands are split only outside quoted values, compiled separately, and coalesced into one review plan. This prevents text inside a note—such as “walking”—from being mistaken for the exercise targeted by a separate completion command, while retaining the existing whole-sentence compiler for tightly coupled commands that share one target.

Every independently compiled direct action also becomes a required action slot keyed by its exact action family, date, and target. Model output and repair output must satisfy every slot and may not introduce an unrelated target. A request to set pain, append quoted note text, and complete Calf Raises is therefore accepted only as those three actions; a plan containing only Walk, or those three plus Walk, is rejected. When the deterministic compiler satisfies all slots, the route returns the review plan directly instead of waiting on provider planning. Quoted payload punctuation is cleaned only at the command boundary and the text is never reinterpreted as an instruction.

Request context separates focal dates from evidence windows. “My ankle hurts today; based on my recent notes…” keeps today as the focal event while retrieving the recent seven-day evidence window. Follow-ups carry bounded prior execution, visual, and action artifacts alongside recent natural-language turns, allowing references such as “that average,” “those days,” “update that response,” or “you only did one action” to retain the artifact being challenged without resending an unbounded transcript. Self-contained analytics requests reset the inherited goal even when they contain words such as “before that.”

Unsupported capabilities are first-class outcomes. If a request needs a derived symptom-event layer that is not available—such as a verified left/right heel trigger chart—and interpretation providers also fail, the route names the missing capability and reports the loaded scope. It does not return a generic overall-pain recap or mark evidence, visualization, action, or date-navigation outputs complete when those artifacts were not produced. Deterministic completion, saved-photo metadata, semantic failure, provider failure, and normal success branches all emit execution status based on actual artifacts.

If semantic terminology providers fail and the user explicitly supplied every requested category, the server produces an exact-wording evidence artifact rather than a total failure. For explicit “exact mention” requests, that deterministic artifact is now the primary path: it counts the supplied surface forms, preserves zero-count categories, exposes source notes for inspection, and clearly states that synonyms were not inferred. Requests that require an AI-derived ontology still fail transparently rather than receiving invented or unrelated counts.
...
The main Ask AI answer budget is currently 950 completion tokens. Groq model chains can be overridden with the bounded `GROQ_MODELS_PTMOTIVATOR_*` environment variables defined in `lib/groq.ts`. Missing and duplicate keys are skipped.
...
Provider scheduling preserves cross-provider diversity. Its first pass spends at most one network attempt on each distinct configured provider; only after those providers had a fair chance may a second pass try alternate keys or models while the shared time and attempt budgets remain. Multiple immediately rate-limited Groq keys therefore cannot consume the complete request before Gemini, Cerebras, or OpenRouter is tried.

Provider failures are classified. A 429 sets an in-memory cooldown; a rejected key is disabled for the process lifetime; and model/request validation failures are not repeated across every key. Every successful provider response is normalized to the existing OpenAI-compatible response shape so route-level validation remains identical. Degraded debug records preserve the scheduling mode and each bounded attempt's provider, model, credential label, status, and short failure detail, allowing the UI archive to distinguish rate limits, rejected credentials, timeouts, contract failures, exhausted budgets, and routes that were never attempted.
...
- Version 1 does not directly edit exercise type-display metadata, exercise program-display metadata, local-only filters, or hidden-completed local state.
- Version 1 represents doctor follow-ups/responses as append operations on the exact doctor note body using a `Response - YYYY-MM-DD` section; it does not create a separate semantic `follow_up` database record or edit voice-transcript tiles.
